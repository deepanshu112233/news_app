...
constructor(props){
  ...
  async componentDidMount(){
    ...
    let url = `https://newsapi.org/v2/top-headlines?country=${this.props.country}&category=${this.props.category}&apiKey=94f319feb39f4cd7a357858c0a386d8f&page=1&pageSize=${this.props.pageSize}`
  ...
  handleBack=async ()=>{
    ...
    let url = `https://newsapi.org/v2/top-headlines?country=${this.props.country}&category=${this.props.category}&apiKey=94f319feb39f4cd7a357858c0a386d8f&page=${this.state.page - 1}&pageSize=${this.props.pageSize}`
    this.setState({loading:true})
    let data = await fetch(url)
    let parsedData = await data.json()
    console.log(parsedData)
  ...
  handleNext=async ()=>{
    ...
    if(!(this.state.page+1> Math.ceil(this.state.totalResults/(this.props.pageSize)))){
      ...
      let url = `https://newsapi.org/v2/top-headlines?country=${this.props.country}&category=${this.props.category}&apiKey=94f319feb39f4cd7a357858c0a386d8f&page=${this.state.page+1}&pageSize=${this.props.pageSize}`
      this.setState({loading:true})
      // data fetch hone se pahle loading show hona chahiye uske bad loading statte ko false kardo
      let data = await fetch(url)
      let parsedData = await data.json()
      console.log(parsedData)
  ...
  render() {
    ...
    return (
      <div className='container'>
        ...
        <br/>  
        <br/>  
          Top Headlines from {this.capitalizeFirst(this.props.category)} </h2>
